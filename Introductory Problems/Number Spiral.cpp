/*
    ...
    Judge: CSES Introductory Problem ( Number Spiral )
    Date : Friday, 22 November 2024, 09:48AM
...
void solve() {
	...
	cin >> n >> m;
	if(n>=m){
		if(!(n%2)) cout << n*n - m + 1 << endl;
		else cout << n*n - 2*n + m + 1 << endl;
	}
	else {
		if(!(m%2)) cout << m*m - 2*m + n + 1 << endl;
