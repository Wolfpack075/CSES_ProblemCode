/*
    ...
    Judge: CSES Introductory Problem ( Tower of Hanoi )
    Date : Saturday, 23 November 2024, 11:49PM
...
void hanoi(int number,int start,int end){
    if(number==1) vec.push_back({start,end});
    else {
        int other = 6 - start - end;
        hanoi(number-1,start,other);
        vec.push_back({start,end});
        hanoi(number-1,other,end);
    }
}
